...
```
workspace/projects/{project}/plans/
└── {plan-name}/
    ├── [C]index-{YYYY-MM-DD}.md          ← INDEX (single entry point)
...
**{project}** is the canonical project slug — e.g., `go-control-erp`, `evonexus-discord-plus`, `cpsmq`, `serket`, `evo-nexus` (for EvoNexus-internal plans). Always ask if unclear.

**Why one folder per plan:** multiple initiatives can have `fase-1-*`, `fase-2-*`, etc. A dedicated `{plan-name}/` folder prevents unrelated phase folders from mixing under the same plans/ directory.

## Step 1 — Create the folder structure

```bash
cd workspace/projects/{project}/plans/
mkdir -p {plan-name}/{phase-1-slug} {plan-name}/{phase-2-slug} {plan-name}/{phase-3-slug}
```

**Plan slug convention:** use the `plan-name` input as the folder slug. It must be lowercase kebab-case.
...
Write `[C]index-{YYYY-MM-DD}.md` inside `workspace/projects/{project}/plans/{plan-name}/`. The index is **a pure navigation file** — no duplication of item content. Template:
...
- Link every item to its own file using relative paths from inside the plan folder

## Step 3 — Write one file per item

For each item across all phases, write `workspace/development/plans/{plan-name}/{phase-slug}/[C]{item-id}-{item-slug}.md` using this template:
...
If a plan folder with the same `plan-name` already exists at `workspace/development/plans/{plan-name}/`:
...
If a legacy plan exists in the old flat format at `workspace/development/plans/[C]{plan-name}-{YYYY-MM-DD}.md` or has `fase-*` folders directly under `workspace/development/plans/`, **do not overwrite or mix with it**. Ask the user whether to migrate it into `workspace/development/plans/{plan-name}/` first.
