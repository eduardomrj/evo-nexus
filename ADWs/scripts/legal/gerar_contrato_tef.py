...
def valor_transacao_extenso(modalidade: str) -> str:
    if modalidade == "pinpad":
        return "quatorze centavos"
    return "vinte e cinco centavos"


def valor_transacao_str(modalidade: str) -> str:
    if modalidade == "pinpad":
        return "0,14"
    return "0,25"
...
def gerar_contrato(
    ...
    # 3. Exibir resumo para confirmação
    ...
    md_content = template.render(
        ...
        valor_transacao          = valor_transacao_str(modalidade),
        valor_transacao_extenso  = valor_transacao_extenso(modalidade),
...
<body>{html_body}</body>
    ...
    cnpj_limpo  = limpar_cnpj(cnpj)
    output_path = OUTPUT_DIR / f"CONTRATO_TEF_{cnpj_limpo}_{data.strftime('%Y-%m-%d')}.pdf"
